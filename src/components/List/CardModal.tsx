...
export default function CardModal({ card, onClose }: CardModalProps) {
  ...
  const actionButtons = [
    ...
    { id: 'labels', label: 'Метки', icon: '🏷', action: () => setShowLabels(!showLabels) },
    { id: 'checklist', label: 'Чек-лист', icon: '✅', action: () => setIsCreatingChecklist(!isCreatingChecklist) },
    { id: 'dates', label: 'Даты', icon: '📅' },
    { id: 'attachment', label: 'Вложения', icon: '📎' },
  ];
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]/80 backdrop-blur-md z-50 flex items-start justify-center p-4 overflow-y-auto animate-in fade-in duration-300" onClick={onClose}>
      <div className="bg-white rounded-2xl border-2 border-cyan-500/30 shadow-[0_0_50px_rgba(6,182,212,0.3)] w-full max-w-2xl my-8 transform animate-in zoom-in-95 duration-300 ease-out" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-cyan-500 px-6 py-4 flex items-center justify-between border-b-2 border-[#0f172a] sticky top-0 z-30 shadow-lg">
          ...
          <button onClick={onClose} className="text-white/80 hover:text-white hover:rotate-90 transition-all duration-300 text-2xl font-bold">✕</button>
        </div>
            ...
            {activeChecklist && (
              <div className="space-y-4 pt-4 border-t border-slate-200 animate-in slide-in-from-left-4">
                <div className="flex items-center justify-between">
                  <label className="text-[10px] font-black text-slate-800 uppercase tracking-widest">✅ {activeChecklist.title}</label>
                  <button onClick={() => setActiveChecklist(null)} className="text-[9px] bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white px-3 py-1 rounded-lg font-black transition-all uppercase">Удалить</button>
                </div>
                ...
                <div key={btn.id} className="relative">
                  ...
                  {btn.id === 'labels' && showLabels && (
                    <div className="absolute left-0 top-full mt-2 w-full bg-white border-2 border-cyan-400 rounded-2xl shadow-2xl p-3 z-40 animate-in zoom-in-95">
                       <p className="text-[9px] font-black text-slate-400 uppercase mb-3 px-1">Метки</p>
